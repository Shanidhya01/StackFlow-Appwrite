...
const Page = async ({ params }: { params: { quesId: string; quesName: string } }) => {
  const [question, answersRaw, upvotes, downvotes, commentsRaw] = await Promise.all([
  ...
  const comments = await Promise.all(
    commentsRaw.documents.map(async (comment) => {
  ...
  const answers = await Promise.all(
    answersRaw.documents.map(async (answer) => {
      const [answerAuthor, answerCommentsRaw, up, down] = await Promise.all([
      ...
      const answerComments = await Promise.all(
        answerCommentsRaw.documents.map(async (comment) => {
          const commentAuthor = await users.get<UserPrefs>(comment.authorId);
          return {
            ...comment,
            author: {
        ...
        <div className="flex">
          <div className="w-full">
            ...
            <div className="flex gap-4 text-sm">
              ...
              <span>
                Score {upvotes.total - downvotes.total} (↑{upvotes.total} ↓{downvotes.total})
              </span>
            </div>
          </div>
